...
% Map-aware planner (known walls/doors + lidar verification)
PLANNER_GRID_RESOLUTION = 0.6;  % [m]
PLANNER_REPLAN_INTERVAL = 0.7;  % [s]
PLANNER_WAYPOINT_TOL    = 0.55; % [m]
PLANNER_NURSE_MARGIN    = 0.95; % [m]
PLANNER_NURSE_YIELD_START = 2.0; % [m] start slowing near nurse traffic
PLANNER_NURSE_YIELD_STOP  = 1.05; % [m] hard-yield distance
PLANNER_NURSE_YIELD_HOLD  = 0.9;  % [s] temporary hold for crossing conflicts
...
seedList = [11];
strategies = struct( ...
    'name', {'conservative_speed', 'aggressive_speed', 'dispatcher_round_robin', 'collision_free_stack'}, ...
    'controlMode', {'conservative_speed', 'aggressive_speed', 'default', 'default'}, ...
    'assignmentPolicy', {'random_item', 'random_item', 'round_robin', 'random_item'}, ...
    'nurseYieldStart', {PLANNER_NURSE_YIELD_START, PLANNER_NURSE_YIELD_START, PLANNER_NURSE_YIELD_START, 2.6}, ...
    'nurseYieldStop', {PLANNER_NURSE_YIELD_STOP, PLANNER_NURSE_YIELD_STOP, PLANNER_NURSE_YIELD_STOP, 1.35}, ...
    'nurseMargin', {PLANNER_NURSE_MARGIN, PLANNER_NURSE_MARGIN, PLANNER_NURSE_MARGIN, 1.2}, ...
    'yieldHold', {PLANNER_NURSE_YIELD_HOLD, PLANNER_NURSE_YIELD_HOLD, PLANNER_NURSE_YIELD_HOLD, 1.4});
...
simCfg = struct( ...
    ...
    'PLANNER_GRID_RESOLUTION', PLANNER_GRID_RESOLUTION, ...
    'PLANNER_REPLAN_INTERVAL', PLANNER_REPLAN_INTERVAL, ...
    'PLANNER_WAYPOINT_TOL', PLANNER_WAYPOINT_TOL, ...
    'PLANNER_NURSE_MARGIN', PLANNER_NURSE_MARGIN, ...
    'PLANNER_NURSE_YIELD_START', PLANNER_NURSE_YIELD_START, ...
    'PLANNER_NURSE_YIELD_STOP', PLANNER_NURSE_YIELD_STOP, ...
    'PLANNER_NURSE_YIELD_HOLD', PLANNER_NURSE_YIELD_HOLD, ...
    'dt', dt, ...
...
isDeterministic = probeA.collisions == probeB.collisions && ...
    probeA.nearMisses == probeB.nearMisses && ...
    probeA.deadlockRecoveries == probeB.deadlockRecoveries && ...
...
function result = runSingleExperiment(strategyCfg, seed, simCfg)
    ...
    runLabel = sprintf('%s | seed=%d', strategyCfg.name, seed);
    env = HospitalEnv(simCfg.WORLD_W, simCfg.WORLD_H, strategyCfg.assignmentPolicy, simCfg.RUN_RENDER, runLabel);
    ...
    for i = 1:simCfg.N_ROBOTS
        ...
        r.configurePlanner(struct( ...
            'gridResolution', simCfg.PLANNER_GRID_RESOLUTION, ...
            'replanInterval', simCfg.PLANNER_REPLAN_INTERVAL, ...
            'waypointTolerance', simCfg.PLANNER_WAYPOINT_TOL, ...
            'nurseSafetyMargin', strategyCfg.nurseMargin, ...
            'nurseYieldStartDist', strategyCfg.nurseYieldStart, ...
            'nurseYieldStopDist', strategyCfg.nurseYieldStop, ...
            'maxYieldHoldTime', strategyCfg.yieldHold));
        env.addRobot(r);
    end

    % Nurses: weighted Hallway-1 / Hallway-2 traffic.
    ...
    while env.SimTime < simCfg.SIM_DURATION
        ...
        if env.allRobotsOutOfOperation()
            fprintf('[STOP %.2fs] All robots are out of operation. Ending run early.\n', ...
                env.SimTime);
            break;
        end
    end
    ...
    result.nearMisses = env.TotalNearMisses;
    result.deadlockRecoveries = env.DeadlockRecoveries;
    result.avgTime = avgTime;
    result.tasksCompleted = numel(env.CompletionTimes);
    result.wallTime = wallTime;

    fprintf('[RESULT] collisions=%d nearMisses=%d deadlockRecoveries=%d avgTime=%.3f completed=%d wall=%.2fs steps=%d\n', ...
        result.collisions, result.nearMisses, result.deadlockRecoveries, ...
        result.avgTime, result.tasksCompleted, result.wallTime, stepCount);
end
...
function printLeaderboard(allResults, strategies)
    ...
    for i = 1:numel(allResults)
        rr = allResults(i);
        fprintf('  %-24s seed=%3d collisions=%4d nearMisses=%4d deadlockRecoveries=%4d avgTime=%8.3f completed=%3d\n', ...
            rr.strategy, rr.seed, rr.collisions, rr.nearMisses, ...
            rr.deadlockRecoveries, rr.avgTime, rr.tasksCompleted);
    end

    summary = repmat(struct('strategy','','meanCollisions',0,'meanNearMisses',0, ...
        'meanDeadlockRecoveries',0,'meanAvgTime',Inf,'meanCompleted',0), ...
        numel(strategies), 1);
    for s = 1:numel(strategies)
        ...
        summary(s).meanNearMisses = mean([subset.nearMisses]);
        summary(s).meanDeadlockRecoveries = mean([subset.deadlockRecoveries]);
    ...
    nearMisses = [summary.meanNearMisses]';
    avgTimes = [summary.meanAvgTime]';
    rankingTable = table((1:numel(summary))', collisions, nearMisses, avgTimes, ...
        'VariableNames', {'idx','collisions','nearMisses','avgTime'});
    rankingTable = sortrows(rankingTable, {'collisions','nearMisses','avgTime'}, {'ascend','ascend','ascend'});

    fprintf('\n===== LEADERBOARD (collisions -> near misses -> avg time) =====\n');
    for rank = 1:height(rankingTable)
        i = rankingTable.idx(rank);
        fprintf('  #%d %-24s collisions=%.2f nearMisses=%.2f deadlockRecoveries=%.2f avgTime=%.3f avgCompleted=%.2f\n', ...
            rank, summary(i).strategy, summary(i).meanCollisions, ...
            summary(i).meanNearMisses, summary(i).meanDeadlockRecoveries, ...
