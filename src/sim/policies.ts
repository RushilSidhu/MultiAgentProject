import { circleRectOverlap, clamp, distance, normalizeAngle, steerAwayFromObstacles } from './geometry';
...
const nominalTargetCommand = (
  ...
  const desired = {
    x: target.x - observation.self.position.x,
    y: target.y - observation.self.position.y,
  };
  ...
  const alignment = Math.max(0.18, Math.cos(headingError));
  const speed = Math.min(observation.self.maxSpeed, distToTarget / 2) * alignment;
...
export class SocialForcePolicy implements RobotPolicy {
  ...
  decide(observation: RobotObservation): RobotAction {
    ...
    const toTarget = { x: target.x - self.position.x, y: target.y - self.position.y };
    ...
    for (const linear of linearSamples) {
      for (const angular of angularSamples) {
        ...
        for (let step = 1; step <= horizon; step += 1) {
          ...
          const targetDist = distance(p, context.target);
          cost += targetDist * targetDist * 2.0 + (linear * linear + angular * angular * 0.12) * 0.18;

          if (
            p.x < self.radius ||
            p.y < self.radius ||
            p.x > observation.scenario.width - self.radius ||
            p.y > observation.scenario.height - self.radius
          ) {
            cost += 2000;
          }
          for (const rect of observation.staticObstacles) {
            if (circleRectOverlap(p, self.radius + 0.12, rect)) cost += 3000;
          }
